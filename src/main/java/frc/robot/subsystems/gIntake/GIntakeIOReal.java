...
public class GIntakeIOReal implements GIntakeIO {
  ...
  public void updateInputs(GIntakeIOInputsAutoLogged inputs) {
    ...
    inputs.rollerSpeedRots = roller.getVelocity().getValueAsDouble();
  }
  ...
  public void runPosition(double setpoint, boolean isIntaking, double feedforward) {
    ...
    // PositionVoltage is a control setting that sets a motor's desired position and applies voltage based on PID and FF to best get to that position
    ...
    you could also just make a new object in the set control function but this is easier to read I tink
    */
    }
}
